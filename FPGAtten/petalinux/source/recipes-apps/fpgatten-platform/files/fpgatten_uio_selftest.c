...
{
    fpgatten_csr_write(csr, FPGATTEN_CSR_CONTEXT_TOKENS, CONTEXT_TOKENS);
...
static int run_hardware_test(const char *gqa_uio, const char *ddr_uio)
{
    ...
    for (uint32_t q_head = 0; q_head < Q_HEADS; ++q_head) {
        uint32_t q_offset = Q_BASE + q_head * FPGATTEN_BF16_HEAD_STRIDE_8192
                            + QUERY_TOKEN * FPGATTEN_BF16_TOKEN_STRIDE;
        uint32_t o_offset = O_BASE + q_head * FPGATTEN_FP32_HEAD_STRIDE_8192
    ...
    for (uint32_t kv_head = 0; kv_head < KV_HEADS; ++kv_head) {
        for (uint32_t token = 0; token < CONTEXT_TOKENS; ++token) {
            uint32_t k_offset = K_BASE + kv_head * FPGATTEN_BF16_HEAD_STRIDE_8192
                                + token * FPGATTEN_BF16_TOKEN_STRIDE;
            uint32_t v_offset = V_BASE + kv_head * FPGATTEN_BF16_HEAD_STRIDE_8192
                                + token * FPGATTEN_BF16_TOKEN_STRIDE;
    ...
    if (wait_for_interrupt(gqa_fd) != 0)
        goto cleanup;
    {
        uint32_t status = fpgatten_csr_read(csr, FPGATTEN_CSR_STATUS);
    ...
    printf("PASS: FPGAtten Z19-P nonzero attention max=%.8f mae=%.8f cycles=%" PRIu32
           ...
           fpgatten_csr_read(csr, FPGATTEN_CSR_OVERLAP_CYCLES),
           fpgatten_csr_read(csr, FPGATTEN_CSR_CORE_QK),
           fpgatten_csr_read(csr, FPGATTEN_CSR_CORE_PV));
    result = 0;
