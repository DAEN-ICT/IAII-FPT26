module gqav5_output_row_buffer #(
...
`ifdef YOSYS
  input  logic [511:0]             write_data_fp32_i,
`else
  input  logic [31:0]              write_data_fp32_i [16],
`endif
...
`ifdef YOSYS
  output logic [511:0]             out_data_fp32_o,
`else
  output logic [31:0]              out_data_fp32_o [16],
`endif
...
`endif
  ...
  logic [511:0] row_mem_q [DEPTH];
  logic out_valid_q;
  logic [OUTPUT_TILE_W-1:0] out_output_tile_q;
  logic [3:0] out_row_index_q;
  logic [511:0] out_data_packed_q;
  logic [511:0] write_data_packed;
  ...
  assign write_fire = write_valid_i;
...
`ifdef YOSYS
      assign write_data_packed[lane * 32 +: 32] =
          write_data_fp32_i[lane * 32 +: 32];
      assign out_data_fp32_o[lane * 32 +: 32] =
          out_data_packed_q[lane * 32 +: 32];
`else
      assign write_data_packed[lane * 32 +: 32] =
          write_data_fp32_i[lane];
      assign out_data_fp32_o[lane] =
          out_data_packed_q[lane * 32 +: 32];
`endif
    end
  endgenerate

  // Reset-free simple-dual-port BRAM template.  Update writes and normalization
  // reads are different downstream FSM phases, so a same-address read/write
  // collision is architecturally impossible and needs no 512-bit fabric
  // bypass mux.  Keeping the two ports in independent clocked processes avoids
  // the old 128x512 store being decomposed into LUTRAM and prevents its write
  // address/data from becoming thousand-load fabric nets.
  ...
  initial begin
    if ((OUTPUT_TILES < 1) || (OUTPUT_TILES > 16) ||
      ...
      $error("output row buffer OUTPUT_TILES must be a power of two in [1,16]");
  end
endmodule
