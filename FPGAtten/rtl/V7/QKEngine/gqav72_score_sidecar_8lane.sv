module gqav72_score_sidecar_8lane #(
  ...
  input  logic [31:0] score_fp32_i [8],
  ...
  input  logic        context_set_i,
  input  logic        context_half_i,
  input  logic [TXN_W-1:0] txn_id_i,

  output logic        out_valid_o,
  input  logic        out_ready_i,
  output logic [31:0] scaled_masked_score_o [8],
  output logic [7:0] score_valid_mask_o,
  output logic [31:0] block_max_o,
  output logic [3:0]  row_index_o,
  output logic        context_set_o,
  output logic        context_half_o,
...
`endif
  ...
  logic        input_valid_q;
  logic [31:0] score_input_q [8];
  logic [31:0] scale_input_q;
  logic [7:0] valid_mask_input_q;
  logic [3:0] row_index_input_q;
  logic context_set_input_q;
  logic context_half_input_q;
  logic [TXN_W-1:0] txn_input_q;
  logic [31:0] scaled_product [8];
  logic        scaled_valid [8];
  logic [7:0] computed_valid_mask;
  logic [MUL_STAGES-1:0] mul_valid_q;
  logic [7:0] mul_valid_mask_q [MUL_STAGES];
  logic [3:0] mul_row_index_q [MUL_STAGES];
  logic mul_context_set_q [MUL_STAGES];
  logic mul_context_half_q [MUL_STAGES];
  logic [TXN_W-1:0] mul_txn_q [MUL_STAGES];

  logic [4:0] pipe_valid_q;
  (* shreg_extract = "no" *) logic [31:0] score_pipe_q [5][8];
  (* shreg_extract = "no" *) logic [7:0] valid_mask_pipe_q [5];
  (* shreg_extract = "no" *) logic [3:0] row_index_pipe_q [5];
  (* shreg_extract = "no" *) logic context_set_pipe_q [5];
  (* shreg_extract = "no" *) logic context_half_pipe_q [5];
  (* shreg_extract = "no" *) logic [TXN_W-1:0] txn_pipe_q [5];
  logic [31:0] pair_max_q [4];
  logic [31:0] quad_max_q [2];
  logic [31:0] block_max_pre_q;
  logic [31:0] block_max_q;
  logic pipeline_advance;
  logic input_capture_advance_local;
  logic mul_meta_advance_local;
  logic scale_advance_local [8];
  ...
  assign context_set_o = context_set_pipe_q[4];
  assign context_half_o = context_half_pipe_q[4];
  assign txn_id_o = txn_pipe_q[4];

  generate
    for (genvar lane = 0; lane < 8; lane++) begin : gen_scale
  ...
  always_ff @(posedge clk_i) begin
    if (input_capture_advance_local && in_valid_i) begin
      ...
      context_set_input_q <= context_set_i;
      context_half_input_q <= context_half_i;
    ...
    if (mul_meta_advance_local) begin
      if (input_valid_q) begin
        ...
        mul_context_set_q[0] <= context_set_input_q;
        mul_context_half_q[0] <= context_half_input_q;
      ...
      for (int stage = 1; stage < MUL_STAGES; stage++) begin
        ...
        mul_context_set_q[stage] <= mul_context_set_q[stage-1];
        mul_context_half_q[stage] <= mul_context_half_q[stage-1];
    ...
    if (payload_advance_local[0] && mul_valid_q[MUL_STAGES-1]) begin
      ...
      context_set_pipe_q[0] <= mul_context_set_q[MUL_STAGES-1];
      context_half_pipe_q[0] <= mul_context_half_q[MUL_STAGES-1];
    ...
    if (payload_advance_local[1] && pipe_valid_q[0]) begin
      ...
      context_set_pipe_q[1] <= context_set_pipe_q[0];
      context_half_pipe_q[1] <= context_half_pipe_q[0];
    ...
    if (payload_advance_local[2] && pipe_valid_q[1]) begin
      ...
      context_set_pipe_q[2] <= context_set_pipe_q[1];
      context_half_pipe_q[2] <= context_half_pipe_q[1];
    ...
    if (payload_advance_local[3] && pipe_valid_q[2]) begin
      ...
      block_max_pre_q <= fp32_max2(quad_max_q[0], quad_max_q[1]);
      valid_mask_pipe_q[3] <= valid_mask_pipe_q[2];
      row_index_pipe_q[3] <= row_index_pipe_q[2];
      context_set_pipe_q[3] <= context_set_pipe_q[2];
      context_half_pipe_q[3] <= context_half_pipe_q[2];
    ...
    if (payload_advance_local[4] && pipe_valid_q[3]) begin
      ...
      // The 8-lane max tree has one fewer arithmetic level than the inherited
      // 16-lane sidecar, but the score/metadata boundary deliberately keeps
      // the proven five-stage contract.  Delay the reduced maximum through
      // that final stage as well; otherwise a full-rate stream pairs row N's
      // score with row N+1's maximum.
      block_max_q <= block_max_pre_q;
      valid_mask_pipe_q[4] <= valid_mask_pipe_q[3];
      row_index_pipe_q[4] <= row_index_pipe_q[3];
      context_set_pipe_q[4] <= context_set_pipe_q[3];
      context_half_pipe_q[4] <= context_half_pipe_q[3];
    ...
    if (rst_ni && pipeline_advance) begin
      for (int lane = 1; lane < 8; lane++) begin
        if (scaled_valid[lane] != scaled_valid[0])
          $error("V7.2 8-lane score-sidecar multiplier valid lanes diverged");
      end
      if (scaled_valid[0] != mul_valid_q[MUL_STAGES-1])
        $error("V7.2 8-lane score-sidecar scale metadata misaligned");
    end
  end
`endif
endmodule
