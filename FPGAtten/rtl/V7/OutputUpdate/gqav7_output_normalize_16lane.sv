module gqav7_output_normalize_16lane #(
  ...
  assign multiply_start = (state_q == ST_RECIP) &&
                          reciprocal_result_valid;
    ...
    if (rst_ni && (state_q == ST_MULTIPLY)) begin
      for (int lane = 1; lane < 16; lane++) begin
        if (normalized_valid_w[lane] != normalized_valid_w[0])
          $error("V7 output-normalize multiplier valid lanes diverged");
      end
    end
  end
`endif
  ...
  initial begin
    if ((STATE_SLOT_W < 1) || (OUTPUT_TILE_W < 1) || (TXN_W < 1))
      $error("V7 output-normalize metadata widths must be positive");
  end
endmodule
