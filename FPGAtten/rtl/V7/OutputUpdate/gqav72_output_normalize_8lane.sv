module gqav72_output_normalize_8lane #(
  ...
  input  logic [31:0]             updated_fp32_i [8],
  ...
  output logic [31:0]             normalized_fp32_o [8],
  ...
  logic row_valid_q;
  logic [31:0] updated_q [8];
  logic [31:0] normalized_q [8];
  logic [31:0] normalized_w [8];
  logic        normalized_valid_w [8];
  ...
  logic multiply_start_q;
  (* keep = "true", dont_touch = "true" *)
  logic [31:0] multiply_reciprocal_q [8];
  (* ram_style = "distributed" *) logic [31:0] reciprocal_cache_q [16];
  logic reciprocal_cache_valid_q [16];
  ...
  generate
    for (genvar lane = 0; lane < 8; lane++) begin : gen_normalize_lane
      gqav7_fp32_mul_rne_pipe i_normalize (
        .clk_i,
        .rst_ni,
        .advance_i     (1'b1),
        .valid_i       (multiply_start_q),
        .a_fp32_i      (updated_q[lane]),
        .b_fp32_i      (multiply_reciprocal_q[lane]),
        .valid_o       (normalized_valid_w[lane]),
  ...
  // Payload is state-qualified and deliberately reset-free, retaining the
  // V5 wide-datapath reset policy.
  always_ff @(posedge clk_i) begin
    ...
    if ((state_q == ST_MULTIPLY) && normalized_valid_w[0]) begin
      for (int lane = 0; lane < 8; lane++)
        normalized_q[lane] <= normalized_w[lane];
    end
  end

  // Wide multiplier payload is deliberately reset-free.  Keeping it out of
  // the asynchronous-reset FSM block prevents Vivado from implementing each
  // bit as an FDCPE with both asynchronous set and clear semantics.  The
  // payload is consumed only when multiply_start_q is asserted one cycle
  // later, so its power-up value is irrelevant.
  always_ff @(posedge clk_i) begin
    if ((state_q == ST_RECIP) && reciprocal_result_valid) begin
      for (int lane = 0; lane < 8; lane++)
        multiply_reciprocal_q[lane] <= reciprocal_result;
    end else if (state_q == ST_REUSE) begin
      for (int lane = 0; lane < 8; lane++)
        multiply_reciprocal_q[lane] <= reciprocal_q;
    end
  end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      for (int row = 0; row < 16; row++)
        reciprocal_cache_valid_q[row] <= 1'b0;
    end else begin
      // Register the launch and replicate the reciprocal at each lane.  This
      // removes the former FSM-state/reciprocal-result mux cone from the DSP
      // inputs.  The extra launch stage changes latency only; throughput and
      // numerical behavior of the multiplier pipeline are unchanged.
      multiply_start_q <= 1'b0;
      ...
      unique case (state_q)
        ST_IDLE: begin
          if (in_valid_i) begin
            ...
            if (!row_valid_i) begin
              ...
              state_q <= ST_OUTPUT;
            end else if (output_tile_i != '0) begin
              if (!reciprocal_cache_valid_q[row_index_i])
                protocol_error_o <= 1'b1;
              reciprocal_q <= reciprocal_cache_q[row_index_i];
              state_q <= ST_REUSE;
        ...
        ST_RECIP: begin
          if (reciprocal_result_valid) begin
            ...
            reciprocal_cache_q[row_index_q] <= reciprocal_result;
            reciprocal_cache_valid_q[row_index_q] <= 1'b1;
...
`ifndef SYNTHESIS
  always_ff @(posedge clk_i) begin
    if (rst_ni && (state_q == ST_MULTIPLY)) begin
      for (int lane = 1; lane < 8; lane++) begin
        if (normalized_valid_w[lane] != normalized_valid_w[0])
          $error("V7.2 8-lane output-normalize multiplier valid lanes diverged");
      end
    end
  end
`endif
  ...
  initial begin
    if ((STATE_SLOT_W < 1) || (OUTPUT_TILE_W < 1) || (TXN_W < 1))
      $error("V7.2 8-lane output-normalize metadata widths must be positive");
  end
endmodule
