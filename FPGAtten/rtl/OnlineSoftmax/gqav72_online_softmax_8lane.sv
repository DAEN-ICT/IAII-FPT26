module gqav72_online_softmax_8lane #(
  ...
  input  logic [7:0]              score_valid_mask_i,
  ...
  output logic [15:0]             probability_bf16_o [8],
  output logic [7:0]              probability_valid_mask_o,
  ...
  logic [31:0] input_score_q [8];

  logic [PIPE_STAGES-1:0] pipe_valid_q;
  pipe_meta_t             meta_q [PIPE_STAGES];

  logic [31:0] score_q [8];
  logic [15:0] probability_q [11:37][8];
  logic [31:0] probability_fp32_q [8];
  ...
  logic [31:0] difference_w [9];
  logic [31:0] scaled_w [9];
  logic [15:0] exp_bf16_w [9];
  logic [31:0] exp_fp32_w [9];
  logic [31:0] sum_pair_w [4];
  logic [31:0] sum_quad_w [2];
  logic [31:0] sum_oct_w [1];
  logic [31:0] block_sum_w;
  logic [31:0] weighted_old_sum_w;
  logic [31:0] running_sum_w;
  logic [8:0]  difference_valid_w;
  logic [8:0]  scale_valid_w;
  logic [3:0]  sum_pair_valid_w;
  logic [1:0]  sum_quad_valid_w;
  logic [0:0]  sum_oct_valid_w;
  ...
  logic        advance_difference_local [9];
  logic        advance_probability_local [8];
  logic        advance_alpha_local;
  logic        advance_pair_local [4];
  logic        advance_quad_local [2];
  logic        advance_oct_local [1];
  logic        advance_block_local;
  logic        advance_weight_local;
  logic        advance_final_local;
  logic        input_fire_local [8];
  logic        stage0_capture;
  logic        stage0_capture_local [8];
  ...
  generate
    for (genvar lane = 0; lane < 8; lane++) begin : gen_difference
    ...
    gqav7_fp32_add_rne_pipe i_alpha_difference (
      .clk_i,
      .rst_ni,
      .advance_i(advance_difference_local[8]),
      .valid_i   (pipe_valid_q[0]),
      .a_fp32_i  (meta_q[0].old_max),
      .b_fp32_i  (neg_new_max_w),
      .valid_o   (difference_valid_w[8]),
      .sum_fp32_o(difference_w[8])
    );

    for (genvar lane = 0; lane < 9; lane++) begin : gen_exp2_scale
    ...
    for (genvar lane = 0; lane < 4; lane++) begin : gen_pair_sum
    ...
    for (genvar lane = 0; lane < 2; lane++) begin : gen_quad_sum
    ...
    for (genvar lane = 0; lane < 1; lane++) begin : gen_oct_sum
  ...
  gqav5_exp2_neg_lut_bank #(.LANES(9)) i_exp2_bank (
    .clk_i,
    .x_fp32_i       (scaled_w),
    .y_bf16_o       (exp_bf16_w),
    .y_fp32_o       (exp_fp32_w),
  ...
  assign context_set_o             = meta_q[37].context_set;
  assign context_half_o            = meta_q[37].context_half;
  ...
  generate
    for (genvar lane = 0; lane < 8; lane++) begin : gen_probability_output
  ...
  generate
    ...
    for (genvar lane = 0; lane < 9; lane++) begin : gen_difference_advance
      gqav5_local_control_buffer i_difference_advance (
        .in_i (advance),
        .out_o(advance_difference_local[lane])
      );
    end
    for (genvar lane = 0; lane < 8; lane++) begin : gen_probability_advance
    ...
    for (genvar lane = 0; lane < 4; lane++) begin : gen_pair_advance
      gqav5_local_control_buffer i_pair_advance (
        .in_i (advance),
        .out_o(advance_pair_local[lane])
      );
    end
    for (genvar lane = 0; lane < 2; lane++) begin : gen_quad_advance
      gqav5_local_control_buffer i_quad_advance (
        .in_i (advance),
        .out_o(advance_quad_local[lane])
      );
    end
    for (genvar lane = 0; lane < 1; lane++) begin : gen_oct_advance
  ...
  always_ff @(posedge clk_i) begin
    if (input_fire) begin
      ...
      input_meta_q.context_set   <= context_set_i;
      input_meta_q.context_half  <= context_half_i;
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (advance_meta_local[0] && input_stage_valid_q) begin
        ...
        meta_q[0].context_set    <= input_meta_q.context_set;
        meta_q[0].context_half   <= input_meta_q.context_half;
    ...
    if (advance_alpha_local) begin
      alpha_q[11] <= meta_q[10].first_context ? FP32_ZERO : exp_fp32_w[8];
      for (int stage = 12; stage <= 37; stage++)
