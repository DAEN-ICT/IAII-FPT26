 ...
 * program uses the existing GQAv7 UIO / PL-DDR ABI; it does not modify RTL,
...
static int parse_options(int argc, char **argv, replay_options *options)
{
    ...
    while (argument < argc) {
        if (strcmp(argv[argument], "--clock-hz") == 0 && argument + 1 < argc) {
            ...
            if (parse_u32(argv[argument + 1], 1u, GQAV5_MAX_CONTEXT_TOKENS,
                          &options->context) != 0)
                return -1;
            argument += 2;
                          ...
                          size_t *o_bytes_o, uint32_t *query_tokens_o)
{
    ...
    if (checked_multiply_size(GQAV5_Q_HEAD_COUNT, query_tokens, &elements) != 0
        || checked_multiply_size(elements, GQAV5_HEAD_DIM, &elements) != 0
        || checked_multiply_size(elements, sizeof(uint16_t), q_bytes_o) != 0)
        return -1;
    if (checked_multiply_size(GQAV5_KV_HEAD_COUNT, options->context, &elements) != 0
        || checked_multiply_size(elements, GQAV5_HEAD_DIM, &elements) != 0
...
static int write_real_inputs(gqav5_device *device, const gqav5_job *job,
                             const replay_options *options,
                             const uint8_t *q, const uint8_t *k,
                             const uint8_t *v, uint32_t query_tokens)
{
    size_t q_head_bytes = (size_t)query_tokens * GQAV5_BF16_TOKEN_STRIDE;
    size_t kv_head_bytes = (size_t)options->context * GQAV5_BF16_TOKEN_STRIDE;
    size_t output_head_bytes = (size_t)query_tokens * GQAV5_FP32_TOKEN_STRIDE;
    uint32_t query_base = options->mode == REPLAY_DECODE
        ? options->context - 1u : 0u;

    for (uint32_t head = 0; head < GQAV5_Q_HEAD_COUNT; ++head) {
        uint32_t offset;

        if (gqav5_row_offset(job->q_base, job->q_head_stride,
                             job->q_token_stride, head, query_base,
                             q_head_bytes, &offset) != 0
            || gqav5_ddr_write(device, offset, q + (size_t)head * q_head_bytes,
                               q_head_bytes) != 0
            || gqav5_row_offset(job->o_base, job->o_head_stride,
                                job->o_token_stride, head, query_base,
                                output_head_bytes, &offset) != 0
            || gqav5_ddr_fill(device, offset, 0u, output_head_bytes) != 0)
            return -1;
    }
    ...
    for (uint32_t head = 0; head < GQAV5_KV_HEAD_COUNT; ++head) {
        uint32_t offset;

        if (gqav5_row_offset(job->k_base, job->k_head_stride,
                             job->k_token_stride, head, 0u,
                             kv_head_bytes, &offset) != 0
            || gqav5_ddr_write(device, offset, k + (size_t)head * kv_head_bytes,
                               kv_head_bytes) != 0
            || gqav5_row_offset(job->v_base, job->v_head_stride,
                                job->v_token_stride, head, 0u,
                                kv_head_bytes, &offset) != 0
            || gqav5_ddr_write(device, offset, v + (size_t)head * kv_head_bytes,
                               kv_head_bytes) != 0)
            return -1;
    }
    return 0;
...
static int read_outputs(const gqav5_device *device, const gqav5_job *job,
                        ...
                        uint32_t query_tokens)
{
    ...
    for (uint32_t head = 0; head < GQAV5_Q_HEAD_COUNT; ++head) {
        ...
        if (gqav5_row_offset(job->o_base, job->o_head_stride,
                             job->o_token_stride, head, query_base,
                             output_head_bytes, &offset) != 0
            || gqav5_ddr_read(device, offset,
                              actual + (size_t)head * query_tokens * GQAV5_HEAD_DIM,
                              output_head_bytes) != 0)
            return -1;
    }
    return 0;
...
static uint64_t unwrap_total_cycles(const gqav5_metrics *metrics,
...
int main(int argc, char **argv)
{
    ...
    if (options.mode == REPLAY_DECODE)
        gqav5_default_decode_job(&job, options.context);
    else
        gqav5_default_prefill_job(&job, options.context, 0u, query_tokens);
    if (gqav5_device_open(&device, options.accelerator_uio, options.ddr_uio) != 0) {
    ...
    printf("FPGATTEN_LLAMA3_DEVICE version=0x%08" PRIx32 " build=0x%08" PRIx32 "\n",
           gqav5_reg_read(&device, GQAV5_CSR_VERSION),
    ...
    for (uint32_t iteration = 0; iteration < options.repeats; ++iteration) {
        ...
        if (gqav5_run_job(&device, &job, 1, 1, options.timeout_ms, &metrics) != 0) {
...
device_cleanup:
    (void)gqav5_soft_reset(&device, 1000u);
