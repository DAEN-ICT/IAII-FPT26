                                  ...
                                  size_t *region_count_o)
{
    ...
    for (uint32_t kv_head = 0; kv_head < KV_HEADS; ++kv_head) {
        uint32_t bytes = maximum_context * GQAV5_BF16_TOKEN_STRIDE;
        if (save_region(ddr, &regions[count++],
                        K_BASE + kv_head * GQAV5_BF16_HEAD_STRIDE_8192,
                        bytes) != 0
            || save_region(ddr, &regions[count++],
                           V_BASE + kv_head * GQAV5_BF16_HEAD_STRIDE_8192,
                           bytes) != 0)
            goto error;
    }
    for (size_t context_index = 0; context_index < context_count;
        ...
        for (uint32_t q_head = 0; q_head < Q_HEADS; ++q_head) {
            if (save_region(ddr, &regions[count++],
                            Q_BASE + q_head * GQAV5_BF16_HEAD_STRIDE_8192
                                + query_token * GQAV5_BF16_TOKEN_STRIDE,
                            GQAV5_BF16_TOKEN_STRIDE) != 0
                || save_region(ddr, &regions[count++],
                               O_BASE + q_head * GQAV5_FP32_HEAD_STRIDE_8192
...
{
    for (uint32_t q_head = 0; q_head < Q_HEADS; ++q_head) {
        uint32_t offset = O_BASE + q_head * GQAV5_FP32_HEAD_STRIDE_8192
                          + query_token * GQAV5_FP32_TOKEN_STRIDE;
...
{
    uint32_t value = gqav5_csr_read(csr, offset);
...
static void program_job(volatile void *csr, uint32_t context)
{
    ...
    gqav5_csr_write(csr, GQAV5_CSR_CONTEXT_TOKENS, context);
                   ...
                   uint32_t context, int verify, struct run_metrics *metrics)
{
    ...
    if (wait_for_interrupt(gqa_fd) != 0) {
        ...
        fprintf(stderr,
                ...
                context, wait_status,
                (wait_status & GQAV5_STATUS_DONE) != 0u ? 1u : 0u,
                (wait_status & GQAV5_STATUS_IRQ) != 0u ? 1u : 0u,
                (wait_status & GQAV5_STATUS_BUSY) != 0u ? 1u : 0u,
                (wait_status & GQAV5_STATUS_ERROR) != 0u ? 1u : 0u,
                wait_total_cycles, wait_core_cycles);
        return -1;
    }
    printf("RUN: context=%" PRIu32 " stage=read_counters\n", context);
    fflush(stdout);
    status = gqav5_csr_read(csr, GQAV5_CSR_STATUS);
    if ((status & (GQAV5_STATUS_DONE | GQAV5_STATUS_IRQ))
        ...
        fprintf(stderr,
                ...
                " done=%u irq=%u busy=%u error=%u\n",
                context, status,
                (status & GQAV5_STATUS_DONE) != 0u ? 1u : 0u,
                (status & GQAV5_STATUS_IRQ) != 0u ? 1u : 0u,
                (status & GQAV5_STATUS_BUSY) != 0u ? 1u : 0u,
                (status & GQAV5_STATUS_ERROR) != 0u ? 1u : 0u);
        return -1;
    }

    metrics->cycles = read_counter(csr, GQAV5_CSR_TOTAL_CYCLES);
    metrics->overlap_cycles = read_counter(csr, GQAV5_CSR_OVERLAP_CYCLES);
    metrics->qk_pv_overlap_cycles =
        read_counter(csr, GQAV5_CSR_QK_PV_OVERLAP);
    metrics->dma_lane_overlap_cycles =
        read_counter(csr, GQAV5_CSR_DMA_LANE_OVERLAP);
    metrics->load_stall_cycles =
        read_counter(csr, GQAV5_CSR_LOAD_STALL_CYCLES);
    metrics->core_total_cycles =
        read_counter(csr, GQAV5_CSR_CORE_TOTAL);
    metrics->qk_cycles = read_counter(csr, GQAV5_CSR_CORE_QK);
    metrics->softmax_cycles = read_counter(csr, GQAV5_CSR_CORE_SOFTMAX);
    metrics->pv_cycles = read_counter(csr, GQAV5_CSR_CORE_PV);
    metrics->dma_stall_cycles =
        read_counter(csr, GQAV5_CSR_CORE_DMA_STALL);
    metrics->dma_ar_count = read_counter(csr, GQAV5_CSR_DMA_AR_COUNT);
    metrics->dma_read_beats =
        read_counter(csr, GQAV5_CSR_DMA_READ_BEATS);
    metrics->dma_ar_wait_cycles =
        read_counter(csr, GQAV5_CSR_DMA_AR_WAIT);
    metrics->dma_r_gap_cycles =
        read_counter(csr, GQAV5_CSR_DMA_R_GAP);
    metrics->dma_r_backpressure_cycles =
        read_counter(csr, GQAV5_CSR_DMA_R_BACKPRESSURE);
    metrics->dma_bank_wait_cycles =
        read_counter(csr, GQAV5_CSR_DMA_BANK_WAIT);
    metrics->dma_emit_wait_cycles =
        read_counter(csr, GQAV5_CSR_DMA_EMIT_WAIT);
    metrics->dma_max_outstanding =
        read_counter(csr, GQAV5_CSR_DMA_MAX_OUTSTANDING);
    metrics->dma_boundary_splits =
        read_counter(csr, GQAV5_CSR_DMA_4K_SPLITS);
    metrics->kv_cache_hits =
        read_counter(csr, GQAV5_CSR_KV_CACHE_HITS);
    metrics->kv_cache_misses =
        read_counter(csr, GQAV5_CSR_KV_CACHE_MISSES);
    metrics->prefetch_issues =
        read_counter(csr, GQAV5_CSR_PREFETCH_ISSUES);
    metrics->blocks_total = read_counter(csr, GQAV5_CSR_PV_BLOCK_TOTAL);
    metrics->blocks_skipped =
        read_counter(csr, GQAV5_CSR_PV_BLOCK_SKIPPED);
            ...
            pv_util_percent = metrics.pv_cycles == 0u ? 0.0
                : 100.0 * (double)(Q_HEADS * summary->context * HEAD_DIM)
                    / ((double)metrics.pv_cycles
                       * ARRAY_MACS_PER_ACTIVE_CYCLE);
            logical_bytes = (double)(Q_HEADS * GQAV5_BF16_TOKEN_STRIDE
                + 2u * KV_HEADS * summary->context
                    * GQAV5_BF16_TOKEN_STRIDE
                + Q_HEADS * GQAV5_FP32_TOKEN_STRIDE);
