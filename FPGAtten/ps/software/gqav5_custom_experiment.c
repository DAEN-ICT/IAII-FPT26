...
static int parse_token_count(const char *text, uint32_t *value_o)
{
    ...
    if (errno != 0 || end == text || *end != '\0'
        || value == 0u || value > GQAV5_MAX_CONTEXT_TOKENS)
        return -1;
...
static int prepare_inputs(gqav5_device *device, const gqav5_job *job)
{
    uint16_t row[GQAV5_HEAD_DIM];
    ...
    for (uint32_t query = 0;
        ...
        for (uint32_t q_head = 0; q_head < GQAV5_Q_HEAD_COUNT; ++q_head) {
            if (gqav5_write_bf16_row(device, job->q_base,
                                     job->q_head_stride,
                                     job->q_token_stride,
                                     q_head, query_token, row) != 0
                || gqav5_clear_output_row(device, job, q_head,
                                          query_token) != 0)
                return -1;
        }
    }

    for (uint32_t kv_head = 0; kv_head < GQAV5_KV_HEAD_COUNT; ++kv_head) {
        for (uint32_t token = 0; token < job->context_tokens; ++token) {
            memset(row, 0, sizeof(row));
            if (gqav5_write_bf16_row(device, job->k_base,
                                     job->k_head_stride,
                                     job->k_token_stride,
                                     kv_head, token, row) != 0)
                return -1;
            for (uint32_t dimension = 0;
                 dimension < GQAV5_HEAD_DIM; ++dimension) {
                float value = (float)(kv_head + 1u)
                    + (float)token / (float)job->context_tokens
                    + (float)dimension / 256.0f;
                row[dimension] = gqav5_float_to_bf16_rne(value);
            }
            if (gqav5_write_bf16_row(device, job->v_base,
                                     job->v_head_stride,
                                     job->v_token_stride,
                                     kv_head, token, row) != 0)
                return -1;
...
static void print_metrics(const gqav5_metrics *metrics, uint64_t clock_hz,
...
static int analyze_outputs(const gqav5_device *device,
                           const gqav5_job *job,
                           float first_output[GQAV5_HEAD_DIM])
{
    float output[GQAV5_HEAD_DIM];
    double visible_sum[GQAV5_KV_HEAD_COUNT][GQAV5_HEAD_DIM];
    ...
    for (uint32_t kv_head = 0;
         kv_head < GQAV5_KV_HEAD_COUNT; ++kv_head) {
        for (uint32_t token = 0; token < initial_visible; ++token) {
            for (uint32_t dimension = 0;
                ...
                visible_sum[kv_head][dimension] +=
                    (double)gqav5_bf16_to_float(
                        gqav5_float_to_bf16_rne(value));
            }
        }
    }
    ...
    for (uint32_t query = 0;
        ...
        for (uint32_t q_head = 0; q_head < GQAV5_Q_HEAD_COUNT; ++q_head) {
            ...
            if (gqav5_read_fp32_row(device, job->o_base,
                                    job->o_head_stride,
                                    job->o_token_stride,
                                    q_head, query_token,
                                    output) != 0)
...
int main(int argc, char **argv)
{
    ...
    if (prefill)
        gqav5_default_prefill_job(&job, context, 0u, query_tokens);
    else
        gqav5_default_decode_job(&job, context);

    if (gqav5_device_open(&device, accelerator_uio, ddr_uio) != 0) {
    ...
    if (gqav5_run_job(&device, &job, 1, 1,
                      prefill ? 120000u : GQAV5_DEFAULT_TIMEOUT_MS,
                      &metrics) != 0) {
