module gqav7_output_update_16lane #(
...
`endif
  ...
  localparam int unsigned STATE_MEM_BANKS = 16;
  localparam int unsigned STATE_MEM_BANK_W = 512 / STATE_MEM_BANKS;
  ...
  logic [31:0]  bypass_state_lane_q [16];
  logic         source_zero_q;
  logic         source_forward_q;
  logic         lane_advance_local [16];
  logic         front_capture_local [16];
  logic         operand_valid_local [16];
  logic         product_alternate_local [16];
  logic         product_alternate_slice_local [16][SOURCE_SELECT_SLICES];
  logic         product_zero_local [16];
  logic         product_zero_slice_local [16][SOURCE_SELECT_SLICES];
  ...
  (* keep = "true", dont_touch = "true" *)
  logic [31:0]  front_alpha_q [16];
  logic [31:0]  partial_input_lane [16];
  logic [31:0]  front_partial_q [16];
  ...
  logic [31:0]  operand_state_q [16];
  logic [31:0]  operand_alternate_q [16];
  (* keep = "true", dont_touch = "true" *)
  logic [31:0]  operand_alpha_q [16];
  logic [31:0]  operand_partial_q [16];
  ...
  logic [31:0]            mul_partial_q [MUL_STAGES][16];
  logic [31:0]            product_w [16];
  logic                   product_valid_w [16];
  logic [31:0]            state_product_w [16];
  logic                   state_product_valid_w [16];
  logic [31:0]            alternate_product_w [16];
  logic                   alternate_product_valid_w [16];
  logic [31:0]            zero_product_w [16];
  logic                   zero_product_valid_w [16];

  logic [ADD_STAGES-1:0] add_valid_q;
  update_meta_t           add_meta_q [ADD_STAGES];
  logic [31:0]            updated_w [16];
  logic                   updated_valid_w [16];
  ...
  generate
    // Sixteen independent 32-bit lane banks preserve the original 512-bit
    // state capacity while allowing each RAMB18 to be placed beside its lane
    // multipliers.  The aggregate memory remains eight BRAM tiles and retains
    // the proven synchronous-read boundary.
      ...
      if (advance) begin
        ...
        if (input_fire) begin
          ...
          accepted_updates_total_o
              <= accepted_updates_total_o + 64'd16;
    ...
    if (rst_ni && !clear_i && advance) begin
      for (int lane = 1; lane < 16; lane++) begin
        if (product_valid_w[lane] != product_valid_w[0])
          $error("V7 output-update multiplier valid lanes diverged");
        if (updated_valid_w[lane] != updated_valid_w[0])
          $error("V7 output-update adder valid lanes diverged");
      end
      for (int lane = 0; lane < 16; lane++) begin
        if (state_product_valid_w[lane] !=
            alternate_product_valid_w[lane])
          $error("V7 output-update parallel multiplier valids diverged");
        if (state_product_valid_w[lane] != zero_product_valid_w[lane])
          $error("V7 output-update zero multiplier valid diverged");
      end
      if (product_valid_w[0] != mul_valid_q[MUL_STAGES-1])
        $error("V7 output-update multiplier metadata misaligned");
      if (updated_valid_w[0] != add_valid_q[ADD_STAGES-1])
        $error("V7 output-update adder metadata misaligned");
      if (source_zero_q && source_forward_q)
        $error("V7 output-update state sources are not one-hot");
    end
  end
`endif
  ...
  initial begin
    if ((STATE_SLOTS < 1) || (STATE_SLOTS > 16) ||
      ...
      $error("V7 output-update STATE_SLOTS must be a power of two in [1,16]");
    if ((OUTPUT_TILES < 1) || (OUTPUT_TILES > 16) ||
        ((OUTPUT_TILES > 1) && ((1 << OUTPUT_TILE_W) != OUTPUT_TILES)))
      $error("V7 output-update OUTPUT_TILES must be a power of two in [1,16]");
  end
endmodule
