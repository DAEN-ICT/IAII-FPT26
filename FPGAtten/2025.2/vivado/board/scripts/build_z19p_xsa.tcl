...
if {[version -short] ne "2025.2"} {
  error "This build requires Vivado 2025.2, running [version -short]"
}
if {![info exists ::env(GQAV7_Z19P_PROJECT_FILE)]} {
  error "Set GQAV7_Z19P_PROJECT_FILE to FPGAtten_z19p.xpr"
}
...
update_ip_catalog
report_ip_status -file [file join $report_dir report_ip_status.rpt]
set locked_ips [get_ips -quiet -filter {IS_LOCKED == 1}]
if {[llength $locked_ips] != 0} {
  error "Vivado 2025.2 project contains locked IPs: $locked_ips"
}
...
# Tcl pre/post hooks must belong to a project fileset or Vivado's
# archive_project cannot copy and rewrite them for a portable archive.
...
if {[llength $reset_cdc_check_object] != 1} {
  error "Expected one reset/CDC implementation hook, found: $reset_cdc_check_object"
}
...
# Retain the proven scoped accelerator OOC DCP binding for the first 2025.2
# migration build.  The migration report records whether 2025.2 still needs
# this compatibility binding before it may be removed in a later revision.
...
# The 2025.2 build enables the strategy's post-route physical optimization by
# default.  This compatibility switch remains available only for a controlled
# comparison with the proven 2024.2 flow.
...
if {$tns eq "NA" && [string is double -strict $wns] && $wns >= 0.0} {
  # Vivado 2025.2 no longer exposes aggregate TNS on get_clocks objects in
  # this design.  A non-negative worst setup slack proves there are no
  # negative setup endpoints, so aggregate TNS is exactly zero.
  set tns "0.000"
}
...
if {$ths eq "NA" && [string is double -strict $whs] && $whs >= 0.0} {
  # The same 2025.2 reporting change applies to aggregate hold slack.
  set ths "0.000"
}
...
set fully_routed_net_count -2
set routing_error_count -1
regexp -nocase {# of routable nets\.+[ \t]*:[ \t]*([0-9]+)[ \t]*:} \
  $route_report_text unused_route_match routable_net_count
regexp -nocase {# of fully routed nets\.+[ \t]*:[ \t]*([0-9]+)[ \t]*:} \
  $route_report_text unused_fully_routed_match fully_routed_net_count
regexp -nocase {# of nets with routing errors\.+[ \t]*:[ \t]*([0-9]+)[ \t]*:} \
  $route_report_text unused_route_error_match routing_error_count
if {$routing_error_count == 0 && $routable_net_count >= 0 &&
    $fully_routed_net_count == $routable_net_count} {
...
puts $metrics_handle "VIVADO_VERSION=[version -short]"
...
puts $metrics_handle "LOCKED_IP_COUNT=[llength [get_ips -quiet -filter {IS_LOCKED == 1}]]"
foreach metrics_ip [get_ips -quiet] {
  set metrics_ip_name [string map {"/" "_" " " "_"} [get_property NAME $metrics_ip]]
  puts $metrics_handle "IP_${metrics_ip_name}_IPDEF=[get_property IPDEF $metrics_ip]"
}
...
file copy [lindex $bit_files 0] [file join $output_dir FPGAtten_2025_2_z19p.bit]
close_project

puts "PASS: FPGAtten Vivado 2025.2 Z19-P bitstream and XSA satisfy DRC/timing gates"
